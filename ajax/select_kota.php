...
if (!empty($_GET['q'])){
	if (ctype_digit($_GET['q'])) {
		...
		while($d = mysql_fetch_array($query)){
			echo "<option value='$d[lokasi_kabupatenkota]&prop=$_GET[q]'>".ucwords(strtolower($d['lokasi_nama']))."</option>";
		}
	}
}
if (empty($_GET['kel']))
{
	if (!empty($_GET['kec']) and !empty($_GET['prop'])){
		...
		include '../db/connection.php';;
			...
			while($d = mysql_fetch_array($query))
			{
			echo "<option value='$d[lokasi_kecamatan]&kec=$d[lokasi_kabupatenkota]&prop=$d[lokasi_propinsi]''>".ucwords(strtolower($d['lokasi_nama']))."</option>";
			}
		}
	}
}
else 
{
	if (!empty($_GET['kec']) and !empty($_GET['prop'])){
		...
		include '../db/connection.php';
			...
			while($d = mysql_fetch_array($query))
			{
				echo "<option value='$d[lokasi_kode]'>".ucwords(strtolower($d['lokasi_nama']))."</option>";
			}
		}
	}
}
